...
지위: ⛔**정합 기준물 생성 — 측정 아님 · 세계 판정 0**. kernel23(정본 폭포 의미론 — FL2.1 문언-동일 승계 · FL2.2 재생성)을
...
★계약(벡터의 읽는 법 — results/frontier_vectors.json `contract`에 동봉):
  ...
  β는 개설(UW 인가) 규칙이지 정산 인자가 아니다. 층 순서 = ①불이행 앵커(앵커) 자유 잔고
...
from kernel23 import World, derive_key                             # noqa: E402
...
def v1():
    """전 층 폭포 단일 청구 — 불이행 앵커 7 → 담보 60 → 소구 15 → 기금 5 → 부족 13."""
    w = World()
    _in(w, "a1", 100)
    ...
    _in(w, "a0", 7)
    return w, "timeout", "전 층 폭포(불이행 앵커 우선→담보→소구→기금→지급불능 기입)"
...
def v4():
    """불이행 앵커 전액 흡수(법 ⑥) — 앵커가 부유하면 보험층 무접촉·담보 전량 반환."""
    w = World()
    _in(w, "a1", 40)
    ref = _redeem(w, "a1", _ln(w, "a1"), "a0")
    _in(w, "a2", 40)
    ...
    _in(w, "a0", 100)
    return w, "timeout", "불이행 앵커 전액 흡수 — 담보·소구·기금 0(초과손해 구조)"
...
def v8():
    ...
    _uw(w, "a4", refs[1], [_ln(w, "a4")], 0)
    _in(w, "a0", 10)
    return w, "timeout", "불이행-층 앵커별 비례 · 인수자-층 무교차(한쪽만 소구 가능)"

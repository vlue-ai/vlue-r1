#!/usr/bin/env python3
"""kernel22_selftest.py — FL2.2 커널 v0.1 게이트 ([M-127] — FL2.1 셀프테스트 18종 전량 승계 + 신설 2).
...
from kernel22 import World, Fl21Error                              # noqa: E402
...
def gate_TTOCOV():
    ...
    out["폭포 손계산 일치"] = rec == {"ref": ref, "comp": 87, "short": 13,
                                "anchor": 7, "cov": 60, "uw": 15, "fund": 5}
...
def gate_TTHROTTLE():
    ...
    out["폭포(부족 = F-자기적용)"] = rec == {"ref": r2, "comp": 8, "short": 4,
                                       "anchor": 0, "cov": 6, "uw": 0, "fund": 2}
    _in(w, "a2", 20)
...
def gate_TPUBREPLAY():
    ...
    pub = World.from_public(pks, "fl22-ref", ("a0", "a1", "a2", "a3"),
                            gen={"redeem_T": 4})
    ...
    pub2 = World.from_public(pks, "fl22-ref", ("a0", "a1", "a2", "a3"),
                             gen={"redeem_T": 4})
    out["결박-변조 검출"] = pub2.replay_verify(bad)["ok"] is False
    bad2 = _c.deepcopy(w.log)
    bad2[2]["head_sig"] = "00" * 64
    pub3 = World.from_public(pks, "fl22-ref", ("a0", "a1", "a2", "a3"),
                             gen={"redeem_T": 4})
...
def main():
    ...
    out["verdict"]["K22_SELFTEST_PASS"] = all(out["verdict"].values())
    os.makedirs(os.path.join(_HERE, "results"), exist_ok=True)
    with open(os.path.join(_HERE, "results", "kernel22_selftest.json"), "w",
              encoding="utf-8") as fh:
        json.dump(out, fh, ensure_ascii=False, indent=1, default=str)
    print(json.dumps(out["verdict"], ensure_ascii=False, indent=1))
    return 0 if out["verdict"]["K22_SELFTEST_PASS"] else 1

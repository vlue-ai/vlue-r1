...
클래스 사다리 v1([VERIFICATION_OBJECT_STUDY §3] — [M-99]):
  ...
  pycheck               코드-이행형 — 약속 = 검사 스크립트(test.py: solution 임포트·검증·
                        ...
                        안에서 실행되므로 미신뢰 산출이 수용 술어를 위조 가능(임포트 시점
                        "OK"+즉시 종료 — 단일-프로세스 수리 원리 불가). 미신뢰 설정은 pyjudge.
  ★pyjudge              ★평가-이행형([M-105] D-10 — 판정-분리 = 사다리 v2의 실물 · RD-9
                        ...
                        (산출 코드 비실행 · rc 0 ∧ "OK") — 미신뢰 산출은 자기 출력만 움직임.
...
def _deps(job):
    """★[M-201] W-4 구성-링크 v0(record-only): 상류 잡 ref 목록 — 스펙에 들어가 H2 로 결박된다(위조-불가)."""
    d = job.get("deps")
    if d is None:
        return None
    if not (isinstance(d, list) and 1 <= len(d) <= 8 and all(
            isinstance(x, str) and len(x) == 16 and all(c in "0123456789abcdef" for c in x) for x in d)):
        raise ValueError("deps = 상류 ref(16-hex) 1~8개")
    if len(set(d)) != len(d):
        raise ValueError("deps 중복")
    return sorted(d)


def validate_spec(job):
    spec = _validate_spec_inner(job)
    d = _deps(job)
    if d is not None:
        spec["deps"] = d
    return spec
...
def _verify_pyjudge(job, output):
    ...
    미신뢰 산출은 자기 출력 바이트만 움직일 수 있다 — 수용 술어는 checker 프로세스 소유."""
    if not isinstance(output, str) or len(output) > PY_MAX_B * 2:
        return False, {"why": "산출 형식"}
    try:
        sol = base64.b64decode(output, validate=True)

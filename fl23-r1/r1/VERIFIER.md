...
- ★[M-217] **커널 성능 패치 규율(세대 내 · v0.2부터)**: 세대 규율은 「커널 해시 동결」이 아니라 **「커널 의미 동결 + 차등 리플레이 게이트」**다.
  세대 안에서 커널 해시가 바뀌었다면 그 패치는 정산법·스키마·REJECT 규칙·`state_root` 공식을 바꾸지 않았어야 하고, 그 증명은 검증자가 직접 재유도한다:
  `python3 fin_lean/lang23/kdiff_check.py` → 동봉 픽스처 두 개(`kdiff_fixture_v01.json` = 이전 커널의 커버리지 원장 · `kdiff_live_fl23_<날짜>.json` = 이전
  커널이 기록한 프로덕션 스냅샷)를 현행 커널의 공개 세계로 전량 리플레이해 **매 항 head·state_root 바이트 동일 · 전량 root == 증분 root · 맵 다이제스트
  == sha256(canon) · 불변식**을 확인(`T_KDIFF_PASS: true`). 하나라도 어긋나면 그 파일은 FL2.3 커널이 아니다(의미 변경 = FL2.4 새 창세 대상). 라이브 대조는
  평소처럼 `replay_full.py --url` — 성능 패치 전후 원장의 head 가 같아야 한다.

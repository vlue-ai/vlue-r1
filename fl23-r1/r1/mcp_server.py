...
def redeem_job(anchor: str, nid: str, kind: str = "sha256_chain",
               ...
               k: int = 0, pk: str = "", msg_sha256: str = "", deps: list = None):
    ...
    if kind in ("sha256_chain", "sha256_chain_sampled"):
        return c.redeem_job(anchor, nid, seed=seed or "ab" * 8, n=int(n),
                            kind=kind, T=Tj, k=(int(k) or None), deps=(deps or None))
    import base64 as _b
    job = {"kind": kind}
    ...
    if deps:                                   # ★W-4 composition link v0 — upstream refs (hash-bound in the spec)
...
@_tool
def hire(kind: str = "sha256_chain", budget: int = 1, deadline_epochs: int = 0,
         anchor: str = "anchor0", seed: str = "", n: int = 5000, checker_py: str = "",
         input_text: str = "", test_py: str = "", deps: list = None):
    """★ONE-CALL HIRE (W-5): pick one of my notes in the anchor's color, split it to the exact budget if
    needed, and order the job. budget = note face in base units (1 AU = meta.unit_scale units; a sha256
    job needs face ≥ ceil(n/250000)). deadline_epochs = 0 → world default (redeem_T); else must exceed
    window_L. Returns {ordered, ref, deadline_epoch, next}. If I hold no note of that color it orders
    nothing and tells me to bootstrap(face) first (mutual-credit swap)."""
    c = _cl()
    budget = int(budget)
    mine = [x for x in c.notes_of(anchor) if int(x["face"]) >= budget]
    if not mine:
        held = sum(int(x["face"]) for x in c.notes_of(anchor))
        return {"ordered": False, "why": f"no {anchor}-colored note with face >= {budget} (held {held})",
                "next": f"bootstrap(face={budget}) to swap my self-IOU for {anchor} notes, then hire again"}
    note = min(mine, key=lambda x: int(x["face"]))
    nid = note["nid"]
    if int(note["face"]) > budget:
        c.split(nid, [budget, int(note["face"]) - budget])
        cand = [x for x in c.notes_of(anchor) if int(x["face"]) == budget]
        nid = max(cand, key=lambda x: int(x["nid"]))["nid"]
    r = _hire_order(c, anchor, nid, kind, seed, int(n), checker_py, test_py, input_text, int(deadline_epochs) or 0, deps)
    ref = (r or {}).get("ref")
    return {"ordered": ref is not None, "ref": ref, "note": nid, "budget": budget,
            "deadline_epoch": (r or {}).get("deadline_epoch"),
            "next": f"hire_status('{ref}') — poll until state != open" if ref else r}


def _hire_order(c, anchor, nid, kind, seed, n, checker_py, test_py, input_text, Tj, deps):
    import base64 as _b
    from sdk import spec_sha256
    if kind in ("sha256_chain", "sha256_chain_sampled"):
        return c.redeem_job(anchor, nid, seed=seed or "ab" * 8, n=n, kind=kind, T=(Tj or None), deps=(deps or None))
    job = {"kind": kind}
    if checker_py:
        job["checker_b64"] = _b.b64encode(checker_py.encode()).decode()
    if test_py:
    ...
    if deps:
        job["deps"] = [str(x) for x in deps]
    args = {"holder": c.p, "note": nid, "anchor": anchor, "spec_sha256": spec_sha256(job)}
    if Tj:
        args["T"] = Tj
    return c._post("/job", {"env": c.sign_env("REDEEM", args), "job": job})


@_tool
def hire_status(ref: str):
    """Plain-English status of a hired job: open (waiting — deadline epoch shown) · delivered (output
    verified — done) · settled_or_returned (deadline missed or cancelled — note returned or compensated)."""
    j = _cl().job(ref)
    st = j.get("state")
    words = {"open": "waiting for the anchor to deliver — if the deadline passes, the ledger settles it automatically",
             "delivered": "done — output verified against the bound spec",
             "settled_or_returned": "not delivered in time (or cancelled) — your note came back / compensation was paid"}
    return {"ref": ref, "state": st, "meaning": words.get(st, "unknown"), "deadline": j.get("deadline"),
            "t0": j.get("t0"), "job": j}


@_tool
def rekey():
    """Rotate MY signing key (FL2.3 REKEY): a new key is generated locally, proof-of-possession is signed,
    the REKEY is submitted; on success the key file is swapped atomically. Pro-active rotation only —
    not a recovery from theft (a thief with the old key could rotate first)."""
    return _cl().rekey()
        ...
        async with stdio_client(params) as (r, w):
            async with ClientSession(r, w) as s:
                ...
                need = {"meta", "join", "bootstrap", "notes", "split",
                        ...
                        "accept_job", "accepts", "hire", "hire_status", "rekey"}   # ★FL2.3 +3

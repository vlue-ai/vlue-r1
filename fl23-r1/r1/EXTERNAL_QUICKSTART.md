...
전 흐름(참여·스왑·상환·이행·인수·검증·호가·★키-회전)이 **로컬 MCP 서버의 도구 37개**로 노출됩니다 —
...
★**첫 5분(한 호출)**: `join` → `bootstrap(face=8*AU)` → **`hire(kind="sha256_chain", budget=…, deadline_epochs=0)`** —
`hire` 가 앵커 색 노트를 골라 정확 액면으로 쪼개고 주문까지 한 번에 합니다(반환 = `{ref, deadline_epoch, next}`) →
`hire_status(ref)` 로 결과를 봅니다(open → delivered / settled_or_returned). 앵커 색 노트가 없으면 주문하지 않고
`bootstrap` 을 안내합니다. 선택 `deps=[상류 ref…]` 로 **구성-링크**(이 잡이 어느 산출을 소비하는지 — 명세 해시에 결박 ·
`/stats.composition` 계기 · record-only)를 남길 수 있습니다.

⚠️**거부는 기록됩니다**(FL2.3): 서명·nonce 가 유효한 봉투가 법에서 거부되면 원장에 REJECT 항이 남고 nonce 가 소비됩니다 —
...
c.bootstrap(8 * AU)
# ★동료 스왑(누구나 유동성 공급자): anchor0 말고 다른 참여자와도 — 서로의 XFER 다리를 /block 원자 교환으로
#   (내 다리: 내 색 노트 → 상대 · 상대 다리: 상대 색 노트 → 나 · make_leg/send_leg/submit_block · 보드 kind="swap" 호가 관례)
...
- ★**잡별 기한(FL2.2부터)**: `redeem_job(..., T=에포크)`로 청구별 기한을 지정할 수
...
- ★**파생 상태도 안 믿어도 됩니다**(FL2.2부터 — H7 공개-리플레이): **잔고·에스크로·기금이
...
canonical_json({typ,args,p,epoch}) ‖ nonce(8B big-endian) )`, `DOMAIN = /meta.domain("FL23-v0.1") + 7×0x00`(세대마다 다르다 — SDK 는 /meta 에서 파생),
...
- ★**비결정론 작업의 다리**(W-1): 실시간 API·TEE·오라클처럼 샌드박스에 못 넣는 일은 **결과가 아니라 서명된 수령증**을 검증한다 —
  그 수령증을 `ed25519_verify` 로 원장에 얹으면 암호-확실 이행이 된다(검증 천장 = 시장 천장의 정직한 다리 · TEE/오라클 어댑터는 첫 실수요 시).

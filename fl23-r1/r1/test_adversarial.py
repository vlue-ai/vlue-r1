...
def _prep(port, data, name="adv"):
    ...
    c.join()
    AU = int(c.meta.get("unit_scale", 1))            # ★단위-인지 — 프로덕션 형상(1000)에서도 같은 배터리
    wk = AnchorWorker(f"http://127.0.0.1:{port}", os.path.join(data, "anchor0.key"))
    g = wk.notes()[0]
    wk.split(g["nid"], [8 * AU, 8 * AU, g["face"] - 16 * AU])   # ★둘 — 하나는 잡, 하나는 예비
    for z in [z for z in wk.notes() if z["face"] == 8 * AU][:2]:
        wk.xfer(name, z["nid"])
    nid = [x["nid"] for x in c.notes_of("anchor0") if x["face"] == 8 * AU][0]
...
def a11_ocommit(port):
    ...
    **열린 sampled ref 에 위조·미서명 `/deliver`**를 쏴, 거부되면서도 원장을 쓰는지 본다."""
    ...
    try:
        ...
        # sampled 청구 하나(공격 표적)
        AU = int(c.meta.get("unit_scale", 1))
        g = wk.notes()[0]
        wk.split(g["nid"], [10 * AU, g["face"] - 10 * AU])
        wk.xfer("oc", [x for x in wk.notes() if x["face"] == 10 * AU][0]["nid"])
        nid = [x["nid"] for x in c.notes_of("anchor0") if x["face"] == 10 * AU][0]
        import jobs as JOBS
...
# ══════════ A-12 합법 SPLIT 팽창 — 상태-상한(J-6) · 무관 참여자 세율 (★FL23_SCOPE §2-③ 재현의 음성 대조) ══════════
def a12_inflate(port):
    """한 합법 신원이 자기 grant 를 조각내 노트 수를 불려 **모든** 참여자의 write 를 과세하던 벡터
    (FL2.2: 1,025 SPLIT → 17×). FL2.3: J-6 상한이 자발 민트를 거부하고(code note_cap) J-5 가 비용을
    O(변경)으로 — 무관 참여자 write 의 세율이 유계여야 한다."""
    import time as _t
    out = {}
    nd, srv, data = _serve(port, notes_per_owner_max=8)
    try:
        c, wk, _ref = _prep(port, data, name="inf")
        def _by_write():
            t0 = _t.perf_counter()
            g = wk.notes()[0]
            wk.split(g["nid"], [1, g["face"] - 1])
            ids = [x["nid"] for x in wk.notes()][:2]
            wk.merge(ids)
            return (_t.perf_counter() - t0)
        base = min(_by_write() for _ in range(3))
        k = 9 - len(c.notes())                                   # 노트 수를 정확히 8(= 상한)로: cnt − 1 + k = 8
        if k < 2:
            ids = [x["nid"] for x in c.notes()][:2]; c.merge(ids); k = 9 - len(c.notes())
        mine = max(c.notes(), key=lambda x: x["face"])
        c.split(mine["nid"], [1] * (k - 1) + [mine["face"] - (k - 1)])
        codes = []
        for _ in range(6):                                       # 더 조각내기 = 상한 초과
            big = max(c.notes(), key=lambda x: x["face"])
            codes.append(_post(port, "/submit", {"env": c.sign_env("SPLIT", {"owner": c.p, "note": big["nid"],
                                                                            "parts": [1, big["face"] - 1]})}))
        out["★상한 초과 SPLIT 전부 거부(code note_cap)"] = all(cd != 200 and "note_cap" in str(m) for cd, m in codes)
        out["★노트 수 유계(≤ 상한)"] = len(c.notes()) <= 8
        after = min(_by_write() for _ in range(3))
        out["무관 참여자 write 세율 ≤ 3×(정보)"] = after <= 3 * base + 0.005
        out["원장 무오염"] = nd.audit()["ok"] is True
    finally:
        srv.shutdown()
    return out
...
# ══════════ A-13 인증-실패 재생 — REJECT 기록 · nonce 소비 · 정직 교정 통과 (J-7 · T-NOLOCK 적대판) ══════════
def a13_reject_replay(port):
    """FL2.2 는 실패-op 가 nonce 를 안 써 같은 봉투를 무한 재생할 수 있었다. FL2.3: 첫 실패 = REJECT 항 1 +
    nonce 소비 → 재생 50회 = 전부 nonce 거부·원장 무성장 · 정직 신원의 교정 op 는 통과(자기-잠금 없음)."""
    out = {}
    nd, srv, data = _serve(port)
    try:
        c, wk, _ref = _prep(port, data, name="rj")
        theirs = wk.notes()[0]["nid"]
        env = c.sign_env("XFER", {"frm": c.p, "to": "anchor0", "note": theirs})   # 미소유 → 인증-거부
        n0 = len(nd.w.log)
        cd1, m1 = _post(port, "/submit", {"env": env})
        out["★첫 실패 = REJECT 항 1 + reject_seq"] = cd1 == 400 and "reject_seq" in str(m1) and len(nd.w.log) == n0 + 1
        codes = [_post(port, "/submit", {"env": env})[0] for _ in range(50)]
        out["★재생 50회 = 전부 거부 · 원장 무성장"] = all(x != 200 for x in codes) and len(nd.w.log) == n0 + 1
        mine = c.notes()[0]
        r = c.split(mine["nid"], [1, mine["face"] - 1])
        out["★정직 교정 op 통과(자기-잠금 없음)"] = isinstance(r, dict) and "seq" in r
        out["원장 무오염"] = nd.audit()["ok"] is True
    finally:
        srv.shutdown()
    return out
...
BATTERY = [("A-1 재생", a1_replay, 8871), ("A-2 교차-세계", a2_crossworld, 8872),
           ...
           ("A-12 합법 팽창·상태-상한(J-6)", a12_inflate, 8883),
           ("A-13 인증-실패 재생(J-7)", a13_reject_replay, 8884)]

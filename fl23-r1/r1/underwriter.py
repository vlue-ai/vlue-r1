...
def scan(c, policy=None):
    ...
    for a in sorted(anchors):
        ...
        for ref in sorted(jc):
            ...
            if lam is not None:
                dv = (st["anchors"].get(a) or {}).get("mature_delivered_volume", 0)
...
def auto_fill(c, policy=None):
    ...
    for m in c.fetch_legs():
        ...
        try:
            ...
            lam = policy.get("trust_lambda")
            if lam is not None:
                dv = (st.get("anchors", {}).get(aj) or {}).get("mature_delivered_volume", 0)
...
def provenance(c):
    ...
    meta = c._get("/meta")
    lang = "lang22" if str(meta.get("domain", "")).startswith("FL22") else "lang23"   # ★세대 선택(아카이브 호환)
    sys.path.insert(0, os.path.join(_HERE, "..", "fin_lean", lang))
    kernel = importlib.import_module("kernel22" if lang == "lang22" else "kernel23")
    pks = {"operator": meta.get("operator_pk0") or meta["operator_pk"], **(meta.get("genesis_pks") or {})}
    w = kernel.World.from_public(pks, meta["label"], tuple(meta["genesis"]),
                                 gen=dict(meta["gen"]),
    ...
    for e in entries:
        ...
        if e.get("kind") == "REJECT":              # ★J-7 — 인증-거부 항: 상태 불변(혈통 무접촉) · 재유도만 확인
            try:
                w._commit(env, replay=True)
                return {"error": f"REJECT 재유도 실패 seq {e.get('seq')}(수용됨) — 리플레이 중단"}
            except kernel.Fl23Error:
                if w.log[-1]["head"] != e["head"]:
...
def acceptance(c, tau=None):
    ...
    try:
        ...
        # ⚠️[M-195] 정직 정정(냉독 라운드5): op_pk 는 **이 노드의 /meta 에서** 온다 —
        # 그래서 이 결박이 이기는 것은 「선언된 정체성 아래 위조 레코드」이지, **자기
        # 정체성(op_pk·log_id)까지 위조하는 완전-악의 노드가 아니다**(그런 노드는 자기
        # op_pk 로 자기-정합 위조 사슬을 서빙한다). 완전-악의 노드의 유일한 방어 =
        # ★**rung-0 아웃-오브-밴드 핀**(/meta 의 operator_pk·log_id 를 발표된 RELEASE·
    ...
    return {"anchors": dict(sorted(per_a.items())),
            ...
            "note": ("수락-집계 — 양측 대칭 기록 = 갈취-레버 차단([M-178] D-5) · "
                     ...
                     "⚠️★sig_verified 는 **이 노드가 선언한 op_pk 기준**이다[M-195]: 자기 "
                     "정체성까지 위조하는 완전-악의 노드는 rung-0(/meta↔RELEASE op_pk·log_id "
                     "아웃-오브-밴드 대조)로만 잡힌다 — acceptance 는 그 핀을 강제하지 않는다)")}

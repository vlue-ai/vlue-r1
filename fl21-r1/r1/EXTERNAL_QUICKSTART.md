...
- ★**퇴장(EXIT) 규칙**: 발행자는 **자기 색 노트가 아직 유통 중이면 퇴장할 수 없습니다**
  (유통 노트를 상환-불능으로 방기하는 「발행자 도주」 차단 — 먼저 전량 상환받거나 회수·
...
- ⚠️**아직 믿어야 하는 것(정직 고지)**: **파생 상태(잔고·에스크로)가 보존 법의 결과인지**는
  v0에서 외부 독립 검증이 안 됩니다(창세 시드가 비밀이라 전-상태 리플레이 불가) — 노드의
  `/audit` 자기-검증에 기댑니다. 상세·완화 경로는 `VERIFIER.md`.
- **믿어야 하는 것(현 단계 정직 고지)**: 단일 시퀀서(검열은 원장 법의 REQUEST/FORCE 강제-
  포함으로 방어) · 발행(자기-IOU·스왑)은 운영자-서명 법 사건 · 공동-서명 키의 분산 보관은
  배포 단계 항목입니다. · ★`/meta`의 공개키들을 **노드에게서 받는 첫 조회는 신뢰-최초-사용
  (TOFU)**입니다 — 엄격한 검증자는 log_id·운영자/공동서명 공개키를 대역-외 채널(공개
  발표문)로 대조하십시오(공개 시 게시 예정).
...
- `pycheck` — 코드-이행(⚠️**협조적-이행자 한정**): 약속 = 검사 스크립트(`test.py`가
  ...
  base64(solution.py))`. ★검사가 산출을 **같은 프로세스에서 실행**하므로 적대적 산출이
...
#   프로파일입니다: ①가해자(앵커) 자산 먼저 → ②담보 → ③인수자 소구 → ④기금.
#   즉 인수자는 **2차-손실 포지션**: 가해자 자산이 부족한 만큼만 담보·소구를 잃습니다
#   (가해자가 충분하면 담보는 반환되고 보험료만 남습니다 — 손익은 stats 손해율로).
# ★배상으로 받는 노트의 색 = **가해-앵커 색**(= 그 앵커에 대한 청구권)입니다 — 앵커가

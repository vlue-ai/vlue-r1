#!/usr/bin/env python3
"""kernel21_selftest.py — FL2.1 커널 v0.1 게이트 ([M-70]).
...
from kernel21 import World, Fl21Error                              # noqa: E402
...
def main():
    ...
    out["verdict"]["K21_SELFTEST_PASS"] = all(out["verdict"].values())
    os.makedirs(os.path.join(_HERE, "results"), exist_ok=True)
    with open(os.path.join(_HERE, "results", "kernel21_selftest.json"), "w",
              encoding="utf-8") as fh:
        json.dump(out, fh, ensure_ascii=False, indent=1, default=str)
    print(json.dumps(out["verdict"], ensure_ascii=False, indent=1))
    return 0 if out["verdict"]["K21_SELFTEST_PASS"] else 1
